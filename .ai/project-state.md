...
> Trạng thái hiện tại tại thời điểm onboarding: **2026-09-20**.
...
| Có chạy được không? | `UNKNOWN` — chưa từng chạy `npm run build` / `npm run dev` trong phiên onboarding |
| Migration DB | **KHÔNG CÓ FILE NÀO** — `supabase/migrations/` rỗng (`VERIFIED`) |
| Trạng thái DB thật | `UNKNOWN` — không truy cập được project Supabase |
...
| F05 | Đăng chuyến + tính giá tự động | Có | `(main)/trips/actions.ts`, `lib/pricing.ts` |
...
| F09 | Chat nội bộ theo chuyến | Có | `trips/[id]/chat/*` |
| F10 | Đánh giá sau chuyến | Một phần | `chat/actions.ts:57` |
...
| U02 | Tên cột `trips.date` hay `trips.trip_date` | Code dùng `date`, tài liệu nói `trip_date` — cần xem DB thật |
